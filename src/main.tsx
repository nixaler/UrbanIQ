...
function StartPage({onBegin,onSelectGame,initialShowSupport,settings}:{onBegin:()=>void,onSelectGame:(gk:string)=>void,initialShowSupport?:boolean,settings?:any}){
  ...
  const [pageStats,setPageStats]=useState({played:0,winPct:0,avg:"—"});
  useEffect(()=>{(async()=>{
    const keys=["pdx","dc","states","nfl","balt","la","nyc","chi"];
    const stats=await Promise.all(keys.map(k=>getFromStorage(`${k}:stats`,{streak:0,wins:0,played:0,totalGuesses:0}).then((s:any)=>s||{})));
    const streaks=stats.map((s:any)=>s?.streak||0);
    setTopStreak(Math.max(...streaks));
    const totalPlayed=stats.reduce((a:number,s:any)=>a+(s?.played||0),0);
    const totalWins=stats.reduce((a:number,s:any)=>a+(s?.wins||0),0);
    const totalGuesses=stats.reduce((a:number,s:any)=>a+(s?.totalGuesses||0),0);
    setPageStats({
      played:totalPlayed,
      winPct:totalPlayed>0?Math.round(totalWins/totalPlayed*100):0,
      avg:totalWins>0?(totalGuesses/totalWins).toFixed(1):"—"
    });
  })();},[]);
...
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@100;200;300;400;700&display=swap" rel="stylesheet"/>
  ...
  /* ── LIGHT MODE V9 — Live Gradients ──────────────────────────────────────── */
  return(
    <div style={{minHeight:"100dvh",background:"#FFFFFF",color:"#0A0A0A",fontFamily:"'Outfit',sans-serif",display:"flex",flexDirection:"column",position:"relative",maxWidth:390,margin:"0 auto",overflowX:"hidden"}}>
      <link rel="manifest" href="/manifest.json"/>
      ...
      <style>{`
        @keyframes v9flow{0%{background-position:0% center}100%{background-position:200% center}}
        @keyframes v9borderFlow{0%{background-position:0% center}100%{background-position:200% center}}
        @keyframes v9barFlow{0%{background-position:-200% center}100%{background-position:200% center}}
        @keyframes v9blink{0%,100%{opacity:1}50%{opacity:.3}}
        .v9-lg{background:linear-gradient(90deg,#0A0A0A 0%,#E8294A 15%,#FF8C42 25%,#FFB800 35%,#4169E1 50%,#A855F7 65%,#E8294A 80%,#0A0A0A 90%,#E8294A 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:v9flow 3s linear infinite;display:inline-block;}
        .v9-lg-fast{background:linear-gradient(90deg,#E8294A 0%,#FF8C42 20%,#FFB800 40%,#FF8C42 60%,#E8294A 80%,#FF8C42 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:v9flow 1.5s linear infinite;display:inline-block;}
        .v9-lg-blue{background:linear-gradient(90deg,#0A0A0A 0%,#4169E1 20%,#A855F7 40%,#4169E1 60%,#0A0A0A 80%,#4169E1 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:v9flow 2.5s linear infinite;display:inline-block;}
        .v9-eyeline{flex:1;height:3px;border-radius:2px;background:linear-gradient(90deg,#E8294A,#FF8C42,#FFB800,#4169E1,#A855F7,#22C55E,#E8294A);background-size:200% auto;animation:v9borderFlow 2s linear infinite;}
        .v9-stats{position:relative;}.v9-stats::after{content:'';position:absolute;bottom:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#E8294A,#FF8C42,#FFB800,#4169E1,#A855F7,#22C55E,#E8294A,#FF8C42);background-size:200% auto;animation:v9borderFlow 2s linear infinite;}
        .v9-bar{background:linear-gradient(90deg,#E8294A,#FF8C42,#FFB800,#4169E1,#A855F7,#E8294A);background-size:300% auto;animation:v9barFlow 2s linear infinite;height:100%;border-radius:2px;width:33%;}
        .v9-dot{width:7px;height:7px;border-radius:50%;background:#E8294A;flex-shrink:0;animation:v9blink 1.5s infinite;}
        .v9-icon{width:38px;height:38px;border-radius:50%;border:1px solid #E8E6E2;display:flex;align-items:center;justify-content:center;font-size:15px;cursor:pointer;background:white;transition:all .2s;}
        .v9-row{display:flex;align-items:center;gap:16px;padding:20px 20px 20px 16px;background:white;cursor:pointer;transition:background .15s;border-bottom:2px solid #E8E5E0;}
        .v9-row:last-child{border-bottom:none;}
        .v9-row:hover{background:#FAFAFA;}
        @media(prefers-color-scheme:dark){body,.v9-icon,.v9-row{background:#FFFFFF!important;color:#0A0A0A!important;}}
      `}</style>
      ...
      {/* NAV */}
      <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 24px",position:"sticky",top:0,zIndex:100,background:"rgba(255,255,255,0.95)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderBottom:"1px solid #EDEBE8"}}>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,letterSpacing:2,color:"#0A0A0A"}}>URBAN<span className="v9-lg">IQ</span></div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <div className="v9-icon" onClick={()=>setShowMaps(true)}>🗺️</div>
          <div className="v9-icon" onClick={()=>setShowBeta(true)}>💬</div>
          {!installed&&<div className="v9-icon" onClick={()=>setShowInstall(true)}>📲</div>}
          {pushSupported&&<div className="v9-icon" onClick={toggleNotif} style={{color:notifEnabled?"#028A48":undefined}}>{notifEnabled?"🔔":"🔕"}</div>}
          <div className="v9-icon" style={{background:isSupporter?"#FFF0F0":"white",borderColor:isSupporter?"#FFD5D5":"#E8E6E2"}} onClick={()=>setShowSupport(true)}>♥</div>
        </div>
      </nav>

      {/* HERO */}
      <div style={{minHeight:"82vh",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 0 48px",position:"relative",overflow:"hidden",background:"#FFFFFF"}}>
        {/* Concentric rings */}
        <div style={{position:"absolute",top:0,left:0,right:0,height:"65%",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
          <div style={{position:"relative",width:300,height:300}}>
            {[40,70,100,130,160,190,220,250,280,310,340,370].map((s,i)=>(
              <div key={i} style={{position:"absolute",width:s,height:s,borderRadius:"50%",border:`1.5px solid rgba(0,0,0,${i>=9?.03:i>=8?.04:i>=7?.05:.07})`,top:"50%",left:"50%",transform:"translate(-50%,-50%)"}}/>
            ))}
          </div>
        </div>
        {/* City skyline SVG */}
        <svg style={{position:"absolute",bottom:0,left:0,right:0,width:"100%",opacity:.07}} viewBox="0 0 390 120" fill="black" xmlns="http://www.w3.org/2000/svg">
          <rect x="10" y="60" width="20" height="60"/><rect x="35" y="40" width="15" height="80"/>
          <rect x="55" y="50" width="25" height="70"/><rect x="85" y="20" width="18" height="100"/>
          <rect x="108" y="35" width="22" height="85"/><rect x="135" y="10" width="30" height="110"/>
          <rect x="170" y="30" width="20" height="90"/><rect x="195" y="45" width="16" height="75"/>
          <rect x="216" y="15" width="28" height="105"/><rect x="249" y="38" width="19" height="82"/>
          <rect x="273" y="55" width="24" height="65"/><rect x="302" y="25" width="22" height="95"/>
          <rect x="329" y="42" width="18" height="78"/><rect x="352" y="60" width="28" height="60"/>
        </svg>
        <div style={{position:"absolute",bottom:0,left:0,right:0,height:"40%",background:"linear-gradient(to top,#FFFFFF 20%,transparent)"}}/>
        {/* Hero text */}
        <div style={{position:"relative",padding:"0 24px"}}>
          <div style={{fontSize:10,fontWeight:600,letterSpacing:3,textTransform:"uppercase",color:"#888580",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
            <span style={{width:24,height:1,background:"#C8C5BF",display:"inline-block"}}/>
            {dateStr} · DAY #{dayNum}
          </div>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:86,lineHeight:.9,letterSpacing:1,marginBottom:4}}>
            <span className="v9-lg">DAILY</span><br/>
            <span className="v9-lg" style={{animationDelay:"-1s"}}>CITY</span><br/>
            <span className="v9-lg" style={{animationDelay:"-2s"}}>QUIZ</span>
          </div>
          <div style={{fontSize:12,fontWeight:400,letterSpacing:2,textTransform:"uppercase",color:"#888580",marginBottom:32,marginTop:12}}>
            TRANSIT · GEOGRAPHY · SPORTS · 5 PUZZLES · 3 ROUNDS
          </div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <div onClick={()=>{SoundEngine.play("select");onSelectGame(hotGameKey);}} style={{flex:1,background:"#0A0A0A",color:"white",fontSize:12,fontWeight:600,letterSpacing:2,textTransform:"uppercase",padding:"16px 20px",borderRadius:4,textAlign:"center",cursor:"pointer",userSelect:"none"}}>PLAY TODAY →</div>
            <div style={{background:"white",border:"1px solid #E8E6E2",fontSize:13,fontWeight:700,padding:"15px 16px",borderRadius:4,display:"flex",alignItems:"center",gap:5,whiteSpace:"nowrap",flexShrink:0}}>
              🔥 <span className="v9-lg-fast">{topStreak||1}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* STATS TICKER */}
      <div className="v9-stats" style={{display:"flex",position:"relative",background:"white"}}>
        {[
          {n:<span className="v9-lg-fast">{topStreak||1}</span>,l:"STREAK"},
          {n:<span className="v9-lg-blue">{pageStats.played||"—"}</span>,l:"PLAYED"},
          {n:<span className="v9-lg-blue" style={{animationDelay:"-.5s"}}>{pageStats.played>0?`${pageStats.winPct}%`:"—"}</span>,l:"WIN %"},
          {n:<span className="v9-lg-blue" style={{animationDelay:"-1s"}}>{pageStats.avg}</span>,l:"AVG"}
        ].map((s,i,a)=>(
          <div key={i} style={{flex:1,padding:"18px 0",textAlign:"center",borderRight:i<a.length-1?"1px solid #EDEBE8":"none"}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:30,letterSpacing:1,lineHeight:1,marginBottom:2}}>{s.n}</div>
            <div style={{fontSize:8,fontWeight:600,letterSpacing:2,textTransform:"uppercase",color:"#C8C5BF"}}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      {(()=>{
        ...
        const g=swipeGames[si];
        return(
          <div style={{padding:"40px 24px 0"}}>
            <div style={{fontSize:9,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"#0A0A0A",marginBottom:20,display:"flex",alignItems:"center",gap:10}}>
              🔥 HOT TODAY <div className="v9-eyeline"/>
            </div>
            <div
              onPointerDown={e=>{hotDragStart.current=e.clientX;(e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);}}
              onPointerUp={e=>{if(hotDragStart.current===null)return;const dx=e.clientX-hotDragStart.current;hotDragStart.current=null;if(Math.abs(dx)<10){SoundEngine.play("select");onSelectGame(g.key);}else if(dx<-40)setHotIdx(p=>(p+1)%swipeGames.length);else if(dx>40)setHotIdx(p=>(p-1+swipeGames.length)%swipeGames.length);}}
              style={{borderRadius:12,overflow:"hidden",marginBottom:12,cursor:"pointer",position:"relative",userSelect:"none",touchAction:"pan-y",minHeight:200}}>
              {g.photo&&<img src={g.photo} alt="" onError={e=>{(e.target as HTMLElement).style.display="none";}} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",filter:"brightness(0.75)"}}/>}
              {!g.photo&&<div style={{position:"absolute",inset:0,background:g.grad}}/>}
              <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,.92) 0%,rgba(0,0,0,.5) 50%,rgba(0,0,0,.15) 100%)"}}/>
              <div style={{padding:"28px 24px",position:"relative",zIndex:1,overflow:"hidden"}}>
                <div style={{position:"absolute",right:-10,bottom:-20,fontFamily:"'Bebas Neue',sans-serif",fontSize:100,color:"rgba(255,255,255,.04)",lineHeight:1,pointerEvents:"none"}}>{g.tag}</div>
                <div style={{fontSize:9,fontWeight:600,letterSpacing:3,textTransform:"uppercase",color:"rgba(255,255,255,.4)",marginBottom:10}}>{g.tag} · {g.sub}</div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:48,letterSpacing:1,lineHeight:.95,marginBottom:12}}>
                  <span style={{background:"linear-gradient(90deg,#fff 0%,#FFB800 20%,#FF8C42 40%,#E8294A 60%,#FFB800 80%,#fff 100%)",backgroundSize:"200% auto",animation:"v9flow 2s linear infinite",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text"}}>{g.name.toUpperCase()}</span>
                </div>
                <div style={{fontSize:11,color:"rgba(255,255,255,.45)",letterSpacing:1,marginBottom:24,textTransform:"uppercase"}}>3 ROUNDS · NEW TODAY · DAY #{dayNum}</div>
                <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"white",color:"#0A0A0A",fontSize:11,fontWeight:700,letterSpacing:2,textTransform:"uppercase",padding:"12px 20px",borderRadius:3}}>PLAY NOW →</div>
              </div>
            </div>
            <div style={{display:"flex",gap:4,alignItems:"center",marginBottom:24}}>
              {swipeGames.map((_,i)=>(
                <div key={i} onClick={()=>setHotIdx(i)} style={{width:i===si?14:5,height:5,borderRadius:3,background:i===si?"#0A0A0A":"rgba(0,0,0,.12)",transition:"all .2s",cursor:"pointer"}}/>
              ))}
            </div>
          </div>
        );
      ...
      {/* ALL GAMES */}
      {(()=>{
        const catMap:{tag:string,label:string,color:string}[]=[
          {tag:"TRANSIT",   label:"Transit",   color:"#4169E1"},
          {tag:"GEOGRAPHY", label:"Geography", color:"#22C55E"},
          {tag:"SPORTS",    label:"Sports",    color:"#E8294A"},
          {tag:"ARCADE",    label:"Arcade",    color:"#A855F7"},
        ];
        const rows=catMap.flatMap(({tag,label,color})=>{
          const cats=tag==="ARCADE"?gameCards.filter(g=>g.key==="minigames"):gameCards.filter(g=>g.tag===tag&&g.key!=="minigames");
          return cats.map(g=>({g,label,color}));
        });
        return(
          <div style={{padding:"0 24px"}}>
            <div style={{fontSize:9,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"#0A0A0A",marginBottom:20,display:"flex",alignItems:"center",gap:10}}>
              ALL GAMES <div className="v9-eyeline"/>
            </div>
            <div style={{display:"flex",flexDirection:"column",background:"white",border:"2px solid #E0DDD8",borderRadius:10,overflow:"hidden",marginBottom:40}}>
              {rows.map(({g,label,color},i)=>(
                <div key={g.key} className="v9-row" onClick={()=>{SoundEngine.play("select");onSelectGame(g.key);}}
                  style={{borderLeft:`5px solid ${color}`,borderBottom:i<rows.length-1?"2px solid #E8E5E0":"none"}}>
                  <div style={{fontSize:24,width:36,textAlign:"center",flexShrink:0}}>{g.emoji}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:9,fontWeight:600,letterSpacing:2,textTransform:"uppercase",color:"#C8C5BF",marginBottom:1}}>{label}</div>
                    <div style={{fontSize:15,fontWeight:700,color:"#0A0A0A"}}>{g.name}</div>
                  </div>
                  <div className="v9-dot"/>
                  <div style={{fontSize:12,color:"#C8C5BF",flexShrink:0}}>→</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {(()=>{
        ...
        return(
          <div style={{padding:"0 24px"}}>
            <div style={{fontSize:9,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"#0A0A0A",marginBottom:20,display:"flex",alignItems:"center",gap:10}}>
              YESTERDAY'S ANSWER <div className="v9-eyeline"/>
            </div>
            <div onClick={()=>{SoundEngine.play("select");onSelectGame(hotGameKey);}} style={{margin:"0 0 40px",padding:20,border:"1px solid #EDEBE8",borderRadius:6,display:"flex",alignItems:"center",gap:16,cursor:"pointer",background:"white"}}>
              <div style={{fontSize:26,flexShrink:0}}>{G2.emoji}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:9,fontWeight:600,letterSpacing:2.5,textTransform:"uppercase",color:"#C8C5BF",marginBottom:3}}>{G2.name}{yest.zone?` · ${yest.zone}`:yest.region?` · ${yest.region}`:""}</div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,letterSpacing:.5,lineHeight:1,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                  <span className="v9-lg">{yest.name.toUpperCase()}</span>
                </div>
                <div style={{fontSize:11,color:"#888580"}}>Could you get it in 1 guess?</div>
              </div>
              <div style={{fontSize:11,fontWeight:600,letterSpacing:1.5,textTransform:"uppercase",color:"#0A0A0A",border:"1px solid #0A0A0A",padding:"10px 14px",borderRadius:3,flexShrink:0,whiteSpace:"nowrap"}}>Play →</div>
            </div>
          </div>
        );
      })()}
      ...
      {/* FOOTER */}
      <div style={{padding:24,borderTop:"1px solid #EDEBE8",display:"flex",justifyContent:"space-between",alignItems:"center",background:"white"}}>
        <div style={{fontSize:10,fontWeight:600,letterSpacing:1.5,textTransform:"uppercase",color:"#C8C5BF"}}>No Ads · No Tracking · Always Free</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,letterSpacing:2,color:"#C8C5BF"}}>NIXALERLLC</div>
      </div>
      {modals}
    </div>
  );
